...
import { LuUsers2, LuFileCheck2 } from "react-icons/lu";
...
//user's info
  ...
  const fetchServiceData = async () => {
    try {
      ...
      const activeServices = allServices.filter((service: ServiceType) => service.isActive);
  ...
  const data = [
    ...
    },
    {
      ...
      value: servicesData.length? (servicesData.length-activeServicesData.length):0,
      bgColor: "bg-[#FFF2E5]",
      ...
      {data.map((item, index) => (
        ...
        >
          ...
          <div className={`ml-4 ${item.textColour}`}>
            <p className={`text-sm font-medium text-gray-800 ${item.textColour}`}>
              {item.title}
            </p>
